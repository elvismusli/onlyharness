...
const importedRoot = path.join(root, "data/imports/smoke-imported-harness");
...
for (const seed of seeds) {
  ...
  const evalDir = path.join(smokeDataRoot, "seed-evals", seed);
  cpSync(dir, evalDir, { recursive: true });
  run("node", [cliBin, "validate", dir, "--strict"]);
  run("node", [cliBin, "eval", evalDir]);
  run("node", [cliBin, "gate", "--dir", evalDir, "--json"]);
}
...
try {
  ...
  for (const route of ["/workspaces/{slug}/workspace", "/workspaces/{slug}/resources", "/workspaces/{slug}/resources/approve", "/workspaces/{slug}/resources/{id}", "/workspaces/{slug}/resources/{id}/archive", "/workspaces/{slug}/collections", "/workspaces/{slug}/collections/{collection}", "/workspaces/{slug}/collections/{collection}/items", "/workspaces/{slug}/collections/{collection}/items/{itemId}", "/workspaces/{slug}/imports/resource-package"]) {
