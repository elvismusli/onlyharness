...
test("private candidate evidence is generated by the packet-bound private fixture smoke", () => {
  const evidenceFileName = "2026-07-13-data-quality-sentinel-0.2.2-client-evidence.json";
  const evidenceText = readFileSync(path.join(evidenceDirectory, evidenceFileName), "utf8");
  const evidence = JSON.parse(evidenceText) as Record<string, any>;
  const packetText = readFileSync(path.resolve(import.meta.dirname, "../data/superskill/review-packets/data-quality-sentinel-0.2.2.json"), "utf8");
  ...
  assert.equal(packet.authorship.immutable, true);
  assert.deepEqual(packet.authorship.author, { actorId: "github-id:149376360", label: "elvismusli" });
  assert.deepEqual(packet.authorship.releaseCutter, { actorId: "github-id:149376360", label: "elvismusli" });
  assert.equal(packet.promotionGate.identityScheme, "github_numeric_actor_v1");
  assert.equal(packet.promotionGate.reviewerActorMustDifferFromAuthor, true);
  assert.equal(packet.promotionGate.reviewerActorMustDifferFromReleaseCutter, true);
  assert.equal(packet.promotionGate.blockedUntilNamedHumanSignoff, true);
  ...
  assert.equal(packet.requiredHumanReview.canonicalPublicReviewerActorRequired, true);
  assert.equal("forbiddenReviewerLabels" in packet.requiredHumanReview, false);

  assert.equal(evidence.schemaVersion, "superskill.exact-release-bootstrap-smoke.v1");
  ...
  assert.equal(evidence.sourceTruthUnchanged, true);
  assert.deepEqual(evidence.release, {
    id: packet.release.id,
    ref: packet.release.ref,
    version: packet.release.version,
    artifactDigest: packet.release.artifactDigest
  });
  assert.deepEqual(evidence.fixtureInput, {
    mode: "private_ignored_snapshot",
    visibility: "private_candidate",
    status: "pending_human_review",
    expiresAt: packet.privateFixture.expiresAt,
    packetBound: true,
  ...
  assert.equal(evidence.clients.length, 2);
  for (const client of evidence.clients) {
    assert.equal(client.realClientSession, "passed");
    assert.equal(client.compatibilitySessionEligible, true);
    assert.equal(client.realClientEvidence.status, "passed");
    assert.equal(client.realClientEvidence.compatibilitySessionEligible, true);
    assert.equal(client.realClientEvidence.publicOutputSanitized, true);
    const packetRow = packet.requiredClientEvidence.find((row: Record<string, unknown>) => row.client === client.client);
    assert.ok(packetRow);
    assert.equal(packetRow.verdict, "pass");
    assert.equal(packetRow.evidenceType, "bootstrap_transport_only");
    assert.equal(packetRow.checkedAt, client.realClientEvidence.checkedAt);
    assert.equal(packetRow.fixtureId, client.realClientEvidence.fixtureId);
  }
  assert.deepEqual(evidence.privacy, {
    tokenPersistedInProjectOrEvents: false,
    taskPersistedInProjectOrEvents: false,
  ...
  assert.doesNotMatch(evidenceText, /OHSC_[A-Za-z0-9_-]{43}/);
  assert.doesNotMatch(packetText, /OHSC_[A-Za-z0-9_-]{43}/);
  for (const pattern of [...secretLikePatterns, ...pathLikePatterns]) {
    assert.doesNotMatch(evidenceText, pattern, evidenceFileName);
    assert.doesNotMatch(packetText, pattern, "private review packet");
  }
  const publicRow = curated.resources.find((resource) => resource.id === packet.release.id);
  assert.ok(publicRow);
  assert.equal(publicRow.version, "0.2.1");
  assert.notEqual(publicRow.expectedDigest, packet.release.artifactDigest);
});
