...
function setupBundleResource(workspace: WorkspaceRecord, resource: resources.Resource, collections: WorkspaceCollection[], target: string): WorkspaceSetupBundleResource {
  ...
  const exactSkillInstall = approvalPin
    ...
    ? `npx --yes onlyharness@0.3.1 resources install ${approvalPin.sourceResourceId} --version ${approvalPin.version} --digest sha256:${approvalPin.archiveHash} --target ${target}${resource.trust.securityScan === "pass" ? "" : " --allow-unreviewed"} --json`
    : undefined;
  return {
    id: resource.id,
    name,
