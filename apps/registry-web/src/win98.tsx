...
export function Btn({ children, onClick, strong, big, pressed, disabled, title, className = "", style }: {
  children: ReactNode;
  onClick?: () => void;
  strong?: boolean;
  big?: boolean;
  ...
  return (
    <button type="button" className={classes} onClick={onClick} disabled={disabled} title={title} style={style}>
      {children}
    </button>
  );
}
