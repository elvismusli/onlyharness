...
/* The Win98 window manager injects its `winView.minimized` view-state into the
   store's deep-link dedup so a repeat fire of the same hash re-focuses a
   minimized deep-link window but leaves an already-visible one untouched —
   preserving the original effect (which depended on `winView` directly and so
   re-ran, and re-focused, when a deep-link window's minimized flag flipped).
   `winMinimized` only re-identifies when the minimized *set* changes, so the
   store effect re-runs on minimize/restore but not on pure position moves. */
function Root98() {
