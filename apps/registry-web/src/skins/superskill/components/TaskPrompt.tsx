...
export function TaskPrompt({ onContinue }: { onContinue: (task: string, client: SuperSkillClient) => void }) {
  ...
  return (
    <form className="ss-task-prompt" onSubmit={submit}>
      ...
      <div className="ss-task-row"><span aria-hidden>$</span><textarea id="ss-task" value={task} maxLength={500} rows={2} onChange={(event) => setTask(event.target.value)} placeholder="Describe the result you need" /><SSButton type="submit">Continue in client</SSButton></div>
