...
- `onlyharness@0.2.14` is a known-bad one-link release because its exact-version npm metadata request is rejected. The shared source is bound to published, official-integrity-pinned `0.2.15`; clean production one-link and new-session plugin proof remain rollout gates until recorded.
