...
- `onlyharness@0.2.14` is a known-bad one-link release because its exact-version npm metadata request is rejected. Source stages `0.2.16` for the public browse MCP plus exact local lifecycle MCP and explicit-consent hosted skill installation into the native Codex/Claude skill root; keep it unavailable until that exact npm release and official integrity are pinned. `0.2.15` remains the previous clean one-link release. Clean new-client marketplace proof remains a rollout gate.
