...
- `onlyharness@0.2.13` is the last clean-`npx` verified CLI. `0.2.14` was published but its one-link installer requests an npm media type rejected by the exact-version endpoint, so it is not public-GO. The shared source is bound to release candidate `0.2.15`; public bootstrap must return `BOOTSTRAP_RELEASE_UNPUBLISHED` until that exact release is published, integrity-pinned, and clean-installed. Marketplace publication and clean new-session plugin proof remain rollout gates.
