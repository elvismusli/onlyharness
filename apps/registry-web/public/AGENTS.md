...
- `onlyharness@0.2.14` is a known-bad one-link release because its exact-version npm metadata request is rejected. The shared source is bound to current public, official-integrity-pinned `0.2.15`; production one-link clean installs into empty Codex and Claude Code projects and the published local MCP exposes the exact eight lifecycle tools. Clean new-client marketplace proof remains a rollout gate.
