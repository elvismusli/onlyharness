...
- Hosted endpoints are not live yet: `hh run` executes locally, and HTTP/MCP surfaces deliver harness files rather than running author code server-side. Do not treat `pricing.model=per_call` as usable until a runner/partner path is shipped and smoked.
