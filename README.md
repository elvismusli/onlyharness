...
- Managed CLI release: `onlyharness@0.2.18` is the current published one-link release, is published and verified through a clean `npx` install, and is pinned to official npm integrity in `plugins/superskill/runtime.json`. It binds hosted skill detail, archive headers and archive bytes to one immutable version/digest/size/trust tuple. `0.2.14` is known-bad; never substitute `latest` for the exact runtime pin.
