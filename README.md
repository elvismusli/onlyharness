...
- Managed CLI release: `onlyharness@0.3.1` is the current one-link transition hotfix, published and pinned to official npm integrity in `plugins/superskill/runtime.json`; clean production one-link/native-install proof remains a rollout gate until the matching API deploy completes. It keeps the local browser-auth broker, binds hosted skill detail/archive bytes to one immutable tuple, and adds anonymous digest-bound native-harness installation. `0.3.0` is the previous transition release. `0.2.14` and `0.2.18` are known-bad one-link releases; never substitute `latest` for the exact runtime pin.
