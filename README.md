...
Resource catalog, immutable `publish-resource`, workspace approval/setup/gates, membership expiry, the manual/provider-agnostic workspace subscription lifecycle, universal install, agent-first browser auth, and digest-bound public skill/harness installs are available in `onlyharness@0.3.1`; `0.3.0` is the previous transition release. `0.2.18` is a known-bad one-link release because its published universal-skill digest is stale. Server catalog flows are also available through MCP/HTTP.
...
- Managed CLI release: `onlyharness@0.3.1` is the current one-link transition hotfix, published and pinned to official npm integrity in `plugins/superskill/runtime.json`. The matching API is deployed, and clean production one-link/native-install checks pass for both Codex and Claude Code, including idempotent `unchanged` replay. It keeps the local browser-auth broker, binds hosted skill detail/archive bytes to one immutable tuple, and adds anonymous digest-bound native-harness installation. `0.3.0` is the previous transition release. `0.2.14` and `0.2.18` are known-bad one-link releases; never substitute `latest` for the exact runtime pin.
