...
- Managed CLI release: `onlyharness@0.3.1` is the current one-link transition hotfix and is pinned to official npm integrity in `plugins/superskill/runtime.json` after publication. It keeps the local browser-auth broker, binds hosted skill detail/archive bytes to one immutable tuple, and adds anonymous digest-bound native-harness installation. `0.3.0` is the previous transition release. `0.2.14` and `0.2.18` are known-bad one-link releases; never substitute `latest` for the exact runtime pin.
