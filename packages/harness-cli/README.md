...
```bash
npx --yes onlyharness@0.2.17 superskill install https://superskill.sh/api/superskill/install --auto
```
...
```bash
npx --yes onlyharness@0.2.17 resources install onlyharness:packages/example-skill --target codex --allow-unreviewed --json
```
