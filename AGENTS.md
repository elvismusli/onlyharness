...
- Runtime source of truth is `plugins/superskill/runtime.json`. `onlyharness@0.3.0` is the current public transition release, pinned to official npm integrity; it adds the local browser-auth broker while keeping the hidden legacy device flow for one release. `0.2.14` and `0.2.18` are known-bad one-link releases. `0.2.19` remains the previous public release. Clean new-client marketplace proof remains a rollout gate.
...
| POST | `/workspaces/{slug}/resources/approve` | Add a public marketplace resource into a workspace catalog/collection as `Approved by {workspace}`; optional `resourceVersion` + `artifactDigest` freeze an exact hosted release; stores a trust snapshot and never grants a SuperSkill Verified badge |
