...
- Runtime source of truth is `plugins/superskill/runtime.json`. `onlyharness@0.2.13` is the last clean-`npx` verified release. `0.2.14` was published but its one-link installer requests an npm media type rejected by the exact-version endpoint, so it is not public-GO. Source stages `0.2.15` with that fix and must keep `cliReleaseStatus=unpublished` plus `cliIntegrity=null` until the exact release is published, integrity-pinned, and clean-installed. Marketplace publication and clean new-session plugin proof remain rollout gates.
