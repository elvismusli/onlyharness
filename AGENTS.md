...
- Runtime source of truth is `plugins/superskill/runtime.json`. `onlyharness@0.2.14` is a known-bad one-link release because its exact-version npm metadata request is rejected. `0.2.15` is published and pinned to the official npm integrity; clean production one-link and new-session plugin proof remain rollout gates until recorded.
