...
- Runtime source of truth is `plugins/superskill/runtime.json`. `onlyharness@0.2.14` is a known-bad one-link release because its exact-version npm metadata request is rejected. Source stages `0.2.16` for the public browse MCP plus exact local lifecycle MCP and explicit-consent hosted skill installation into the native Codex/Claude skill root; keep `cliReleaseStatus=unpublished` plus `cliIntegrity=null` until that exact release is published and its official npm integrity is pinned. `0.2.15` remains the previous clean one-link release. Clean new-client marketplace proof remains a rollout gate.
