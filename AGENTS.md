...
- Runtime source of truth is `plugins/superskill/runtime.json`. `onlyharness@0.2.14` is a known-bad one-link release because its exact-version npm metadata request is rejected. `0.2.15` is the current public release, pinned to the official npm integrity and clean-installed from the production one-link into empty Codex and Claude Code projects; its published `superskill_local` stdio server exposes the exact eight lifecycle tools. Clean new-client marketplace proof remains a rollout gate.
