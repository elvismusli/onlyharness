...
- Runtime source of truth is `plugins/superskill/runtime.json`. `onlyharness@0.3.1` is the current public transition hotfix, pinned to official npm integrity after publication; it keeps the local browser-auth broker and adds digest-bound anonymous native-harness installation for the public catalog. `0.3.0` is the previous transition release. `0.2.14` and `0.2.18` are known-bad one-link releases. Clean new-client marketplace proof remains a rollout gate.
