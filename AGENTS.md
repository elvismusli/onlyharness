...
- Runtime source of truth is `plugins/superskill/runtime.json`. `onlyharness@0.2.14` is a known-bad one-link release because its exact-version npm metadata request is rejected. Source stages `0.2.17` to make planned/unchanged catalog install copy status-accurate; keep `cliReleaseStatus=unpublished` plus `cliIntegrity=null` until that exact npm release and official integrity are pinned. `0.2.16` remains the current public release with dual MCP one-link and immutable native hosted-skill installation. Clean new-client marketplace proof remains a rollout gate.
