...
- Hosted execution endpoints are not live. `hh run` stays local-only, MCP/HTTP archive routes deliver files, and `pricing.model=per_call` must not be marketed as usable until a partner-backed or first-party runner has passed runtime smoke.
