...
- Runtime source of truth is `plugins/superskill/runtime.json`. `onlyharness@0.2.14` is a known-bad one-link release because its exact-version npm metadata request is rejected. `0.2.16` is the current public release, pinned to official npm integrity; its one-link installs both the public browse MCP and exact local lifecycle MCP, and its explicit-consent catalog path installs hosted skills into the native Codex/Claude skill root with immutable digest verification. `0.2.15` remains the previous clean one-link release. Clean new-client marketplace proof remains a rollout gate.
